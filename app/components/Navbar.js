    ...
    <nav className="bg-yellow-600 text-white shadow-md">
      <div className="container mx-auto flex justify-between items-center p-1">
        ...
        <Link href="/" className="text-2xl font-bold flex items-center">
          <img src="../icon.png" alt="Logo" width="55" height="55" className="d-inline-block align-text-top m-2" />
          RecipeBook
        </Link>
        ...
        {user.value && (
          <div className="relative group mr-4 md:mr-0">
        ...
        {!user.value && (
          <Link href="/login" className="text-xl font-bold mr-4 md:mr-0">Log in</Link>
        )}
      </div>

      {/* Side Panel Menu - Left side */}
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-yellow-800 text-white transform ${isOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out z-50`}
      >
        <button
          onClick={toggleMenu}
          className="absolute top-4 left-4 focus:outline-none text-white"
        >
